...
Deno.serve(async (req) => {
  try {
    ...
    // Sifarişi al
    const sifaris = await db.asServiceRole.entities.Sifaris.get(sifaris_id);
    if (!sifaris) {
      return Response.json({ error: 'Sifariş tapılmadı' }, { status: 404 });
    }

    // Aktiv kassaları al — birincini default götür
    const kassalar = await db.asServiceRole.entities.Kassa.filter({ status: 'Aktiv' });
    ...
    const movcut = await db.asServiceRole.entities.KassaEmeliyyati.filter({ sifaris_id: sifaris_id });
    ...
    if (odenis_statusu === 'Ödənilib') {
      ...
      if (qalan <= 0) {
        // Sifaris-də odenilmis_mebleg-i tam məbləğə qur
        await db.asServiceRole.entities.Sifaris.update(sifaris_id, {
          odenilmis_mebleg: tamMebleg,
        }).catch(() => null);
        return Response.json({ message: 'Bu sifariş artıq tam ödənilib' });
      }
      // Yalnız qalan hissəni yaz
      await db.asServiceRole.entities.KassaEmeliyyati.create({
        kassa_id: kassa.id,
        tip: 'Mədaxil',
      ...
      await db.asServiceRole.entities.Kassa.update(kassa.id, { balans: yeniBalans });
      // Sifaris-də odenilmis_mebleg-i tam məbləğə qur
      await db.asServiceRole.entities.Sifaris.update(sifaris_id, {
        odenilmis_mebleg: tamMebleg,
      }).catch(() => null);
      const jurnal_no2 = `J-${Date.now().toString().slice(-6)}`;
      await db.asServiceRole.entities.JurnalQeydi.create({
    ...
    // Sifariş entity-sinə ödənilmiş məbləği yaz (kumulativ)
    const artiqOdenilmis = movcudlar.reduce((s, e) => s + (e.mebleg || 0), 0);
    const yeniOdenilmis = artiqOdenilmis + mebleg;
    await db.asServiceRole.entities.Sifaris.update(sifaris_id, {
      odenilmis_mebleg: yeniOdenilmis,
    });
