...
Deno.serve(async (req) => {
  try {
    ...
    const sifarisler = await db.asServiceRole.entities.Sifaris.filter({ status: "Yeni" }, "-created_date", 20);
    const thirtyMinsAgo = new Date(Date.now() - 30 * 60 * 1000).toISOString();
    for (const s of sifarisler) {
      if (s.created_date > thirtyMinsAgo) {
        const existing = await db.asServiceRole.entities.Bildiris.filter({ istinad_id: s.id, tip: "Yeni sifariş" }, "-tarix", 1);
        if (existing.length === 0) {
          await db.asServiceRole.entities.Bildiris.create({
    ...
    const anbar = await db.asServiceRole.entities.Anbar.list("-created_date", 200);
    for (const item of anbar) {
      if ((item.miqdar || 0) <= (item.min_miqdar || 5)) {
        const existing = await db.asServiceRole.entities.Bildiris.filter({ istinad_id: item.id, tip: "Stok xəbərdarlığı" }, "-tarix", 1);
        if (existing.length === 0) {
          await db.asServiceRole.entities.Bildiris.create({
    ...
    const sikayetler = await db.asServiceRole.entities.Sikayet.filter({ status: "Açıq" }, "-created_date", 50);
    for (const s of sikayetler) {
      if (s.prioritet === "Yüksək" || s.prioritet === "Kritik") {
        const existing = await db.asServiceRole.entities.Bildiris.filter({ istinad_id: s.id, tip: "Sikayet" }, "-tarix", 1);
        if (existing.length === 0) {
          await db.asServiceRole.entities.Bildiris.create({
    ...
    const fakturalar = await db.asServiceRole.entities.Faktura.list("-created_date", 200);
    for (const f of fakturalar) {
      if (f.odenis_statusu !== "Ödənilib" && f.son_odenis_tarixi && f.son_odenis_tarixi <= threeDaysLater && f.son_odenis_tarixi >= today) {
        const existing = await db.asServiceRole.entities.Bildiris.filter({ istinad_id: f.id, tip: "Ödəniş" }, "-tarix", 1);
        if (existing.length === 0) {
          await db.asServiceRole.entities.Bildiris.create({
    ...
    const avadanliq = await db.asServiceRole.entities.Avadanliq.list("-created_date", 200);
    for (const a of avadanliq) {
      if (a.novbeti_baxim && a.novbeti_baxim <= threeDaysLater && a.novbeti_baxim >= today) {
        const existing = await db.asServiceRole.entities.Bildiris.filter({ istinad_id: a.id, tip: "Avadanlıq baxım" }, "-tarix", 1);
        if (existing.length === 0) {
          await db.asServiceRole.entities.Bildiris.create({
