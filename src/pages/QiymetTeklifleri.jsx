...
export default function QiymetTeklifleri() {
  ...
  const fetchData = () => {
    db.entities.QiymetTeklifi.list("-created_date", 100).catch(() => []).then(data => {
  ...
  const handleCreate = async () => {
    ...
    await db.entities.QiymetTeklifi.create({
      teklif_no: no,
  ...
  const handleStatusChange = async (id, status) => {
    await db.entities.QiymetTeklifi.update(id, { status });
    fetchData();
  };
                ...
                <tr key={t.id} className="border-t border-border/50 hover:bg-muted/20">
                  ...
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-1">
                      ...
                      {isAdmin && <DeleteButton onDelete={async () => { await db.entities.QiymetTeklifi.delete(t.id); fetchData(); }} />}
                    </div>
                  </td>
                </tr>
              ))}
