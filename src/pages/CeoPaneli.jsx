  ...
  useEffect(() => {
    const load = async () => {
      ...
      const [sifarisler, kassaEmeliyyatlari, iscilar, maaslar, podratciSifarisleri, anbar] = await Promise.all([
        ...
        db.entities.KassaEmeliyyati.list("-tarix", 1000).catch(() => []),
        db.entities.Isci.list().catch(() => []),
        db.entities.MaasHesablamasi.list().catch(() => []),
        db.entities.PodratciSifarisi.list("-xidmet_tarixi", 500).catch(() => []),
        db.entities.Anbar.list().catch(() => []),
      ]);
