...
export default function PodratciHesabati() {
  ...
  const fetchData = async () => {
    const [p, s, p2] = await Promise.all([
      db.entities.PodratciSifarisi.list("-xidmet_tarixi", 200).catch(() => []),
      db.entities.Sifaris.list().catch(() => []),
      db.entities.Podratci.list().catch(() => []),
    ]);
    setSifarisler(s);
    setPodratcilar2(p2);
  ...
  const handleAddSave = async () => {
    ...
    await db.entities.PodratciSifarisi.create({
      ...form,
      isin_deyeri,
      podratci_payi,
  ...
  const handleStatusChange = async (id, status) => {
    await db.entities.PodratciSifarisi.update(id, { odenis_statusu: status });
    fetchData();
  };

  const handleDelete = async (id) => {
    await db.entities.PodratciSifarisi.delete(id);
    fetchData();
  };
