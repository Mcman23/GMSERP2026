...
export default function Iscilar() {
  ...
  const fetchData = () => { db.entities.Isci.list("-created_date", 100).then(setIscilar).finally(() => setLoading(false)); };
  useEffect(() => { fetchData(); }, []);

  const handleCreate = async () => {
    await db.entities.Isci.create({ ...form, maas: parseFloat(form.maas) || 0, status: "Aktiv" });
    setShowDialog(false);
        ...
        {iscilar.map(i => (
          <div key={i.id} className="bg-card rounded-xl border border-border p-5 hover:shadow-md transition-shadow">
            ...
            <div className="flex justify-between items-center mt-3 pt-3 border-t border-border/50 text-xs text-muted-foreground">
              ...
              {isAdmin && <DeleteButton onDelete={async () => { await db.entities.Isci.delete(i.id); fetchData(); }} />}
            </div>
          </div>
        ))}
      </div>
