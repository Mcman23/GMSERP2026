...
export default function Podratcilar() {
  ...
  const fetchData = () => db.entities.Podratci.list().then(d => { setList(d); setLoading(false); });
  useEffect(() => { fetchData(); }, []);
  ...
  const handleSave = async () => {
    ...
    if (editing) {
      await db.entities.Podratci.update(editing.id, data);
    } else {
      await db.entities.Podratci.create(data);
    }
    setShowDialog(false);
    fetchData();
  };
  ...
  const handleDelete = async (id) => {
    ...
    await db.entities.Podratci.delete(id);
    fetchData();
  };
