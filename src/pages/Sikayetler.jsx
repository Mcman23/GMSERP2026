...
export default function Sikayetler() {
  ...
  const fetchData = () => {
    Promise.all([
      db.entities.Sikayet.list("-created_date", 100),
  ...
  const handleCreate = async () => {
    ...
    await db.entities.Sikayet.create({
  ...
  const handleStatusChange = async (id, status) => {
    await db.entities.Sikayet.update(id, { status });
    fetchData();
  };
                ...
                <tr key={s.id} className="border-t border-border/50 hover:bg-muted/30">
                  ...
                  <td className="px-4 py-3 text-right">
                    <div className="flex items-center justify-end gap-1">
                      ...
                      {isAdmin && <DeleteButton onDelete={async () => { await db.entities.Sikayet.delete(s.id); fetchData(); }} />}
                    </div>
                  </td>
                </tr>
              ))}
