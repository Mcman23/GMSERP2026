...
export default function Fakturalar() {
  ...
  const fetchData = () => {
    Promise.all([
      db.entities.Faktura.list("-created_date", 100),
  ...
  const handleCreate = async () => {
    ...
    await db.entities.Faktura.create({
            ...
            <tbody>
              {fakturalar.map(f => (
                <tr key={f.id} className="border-t border-border/50 hover:bg-muted/30">
                  ...
                  {isAdmin && <td className="px-2 py-3"><DeleteButton onDelete={async () => { await db.entities.Faktura.delete(f.id); fetchData(); }} /></td>}
                  </tr>
              ))}
