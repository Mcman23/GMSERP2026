...
export default function Avadanliq() {
  ...
  const fetchData = () => { db.entities.Avadanliq.list("-created_date", 100).then(setItems).finally(() => setLoading(false)); };
  useEffect(() => { fetchData(); }, []);

  const handleCreate = async () => {
    await db.entities.Avadanliq.create({
        ...
        {items.map(i => (
          <div key={i.id} className="bg-card rounded-xl border border-border p-5 hover:shadow-md transition-shadow">
            ...
            <div className="mt-3 pt-3 border-t border-border/50 text-xs flex justify-between items-center">
              ...
              {isAdmin && <DeleteButton onDelete={async () => { await db.entities.Avadanliq.delete(i.id); fetchData(); }} />}
            </div>
          </div>
        ))}
      </div>
