...
export default function Maas() {
  ...
  useEffect(() => {
    Promise.all([
      db.entities.Isci.list(),
      db.entities.MaasHesablamasi.list("-created_date", 100).catch(() => []),
    ]).then(([i, h]) => { setIscilar(i.filter(x => x.status === "Aktiv")); setHesablamalar(h); setLoading(false); });
  }, []);

  const fetchHesablamalar = () => {
    db.entities.MaasHesablamasi.list("-created_date", 100).catch(() => []).then(setHesablamalar);
  };
  ...
  const handleSave = async () => {
    ...
    await db.entities.MaasHesablamasi.create({
  ...
  const handleOdendi = async (id) => {
    await db.entities.MaasHesablamasi.update(id, { status: "Ödənildi", odenis_statusu: "Ödənilib" });
    fetchHesablamalar();
  };

  const handleDelete = async (id) => {
    await db.entities.MaasHesablamasi.delete(id);
    fetchHesablamalar();
  };
