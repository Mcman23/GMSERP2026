...
export default function Kassa() {
  ...
  const fetchData = () => {
    Promise.all([
      db.entities.Kassa.list(),
      db.entities.KassaEmeliyyati.list("-created_date", 50),
    ]).then(([k, e]) => {
      setKassalar(k);
      setEmeliyyatlar(e);
      setLoading(false);
  ...
  const handleKassaCreate = async () => {
    await db.entities.Kassa.create({ ...kassaForm, balans: 0, status: "Aktiv" });
  ...
  const handleEmeliyyat = async () => {
    ...
    await db.entities.Kassa.update(kassa.id, { balans: yeniBalans });
  ...
  const handleKassaBaglama = async (kassa) => {
    await db.entities.Kassa.update(kassa.id, {
            ...
            {kassalar.map(kassa => (
              <div key={kassa.id} className="bg-card rounded-xl border border-border p-5 hover:shadow-md transition-shadow">
                ...
                <div className="flex gap-2 mt-4">
                  ...
                  {isAdmin && <DeleteButton onDelete={async () => { await db.entities.Kassa.delete(kassa.id); fetchData(); }} />}
                </div>
              </div>
            ))}
          </div>
                  ...
                  {emeliyyatlar.map(em => {
                    ...
                    return (
                      <tr key={em.id} className="border-t border-border/50 hover:bg-muted/30">
                        ...
                        {isAdmin && <td className="px-2 py-3"><DeleteButton onDelete={async () => { await db.entities.KassaEmeliyyati.delete(em.id); fetchData(); }} /></td>}
                      </tr>
                    );
                  })}
