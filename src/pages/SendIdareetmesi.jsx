...
export default function SendIdareetmesi() {
  ...
  const fetchData = () => {
    Promise.all([
      db.entities.Sened.list("-created_date", 200).catch(() => []),
      db.entities.Musteri.list().catch(() => []),
      db.entities.Sifaris.list("-tarix", 300).catch(() => []),
      db.entities.KassaEmeliyyati.list("-tarix", 300).catch(() => []),
    ]).then(([s, m, sf, k]) => {
  ...
  const handleCreate = async () => {
    ...
    await db.entities.Sened.create({
      ...form,
  ...
  const handleDelete = async (sened) => {
    await db.entities.Sened.update(sened.id, { status: "Silindi" });
    fetchData();
