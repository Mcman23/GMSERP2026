...
export default function Anbar() {
  ...
  const fetchData = () => { db.entities.Anbar.list("-created_date", 200).then(setItems).finally(() => setLoading(false)); };
  useEffect(() => { fetchData(); }, []);

  const handleCreate = async () => {
    const miqdar = parseFloat(form.miqdar) || 0;
    const minMiqdar = parseFloat(form.min_miqdar) || 5;
    await db.entities.Anbar.create({
            ...
            <tbody>
              {items.map(i => (
                <tr key={i.id} className={`border-t border-border/50 hover:bg-muted/30 ${i.miqdar <= i.min_miqdar ? "bg-orange-50/50" : ""}`}>
                  ...
                  {isAdmin && <td className="px-2 py-3"><DeleteButton onDelete={async () => { await db.entities.Anbar.delete(i.id); fetchData(); }} /></td>}
                </tr>
              ))}
