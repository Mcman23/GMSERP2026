...
export default function Musteriler() {
  ...
  const fetchData = () => {
    db.entities.Musteri.list("-created_date", 100).then(setMusteriler).finally(() => setLoading(false));
  };

  useEffect(() => { fetchData(); }, []);

  const handleCreate = async () => {
    await db.entities.Musteri.create({ ...form, status: "Aktiv" });
    setShowDialog(false);
        ...
        {filtered.map(m => (
          <div key={m.id} className="bg-card rounded-xl border border-border p-5 hover:shadow-md transition-shadow">
            ...
            <div className="flex justify-between items-center mt-3 pt-3 border-t border-border/50 text-xs text-muted-foreground">
              ...
              {isAdmin && <DeleteButton onDelete={async () => { await db.entities.Musteri.delete(m.id); fetchData(); }} />}
            </div>
          </div>
        ))}
      </div>
