...
export default function SirketDovriiyyesi({ musteriler, sifarisler, kassaEmeliyyatlar }) {
  ...
  // Sifariş üçün real ödənilən məbləği al:
  // 1) Tam ödənilibsə — ümumi məbləği qaytar
  // 2) Qismən ödənilibsə — əvvəlcə entity-dəki odenilmis_mebleg-ə bax,
  //    yoxdursa kassa əməliyyatlarından hesabla
  // 3) Ödənilməyibsə — 0
  const getSifarisOdenilen = (sifaris) => {
    if (sifaris.odenis_statusu === "Ödənilib") {
      return sifaris.umumi_mebleg || sifaris.qiymet || 0;
    }
    if (sifaris.odenis_statusu === "Qismən ödənilib") {
      if (sifaris.odenilmis_mebleg && sifaris.odenilmis_mebleg > 0) {
        return sifaris.odenilmis_mebleg;
      }
      // Fallback: kassa əməliyyatlarından hesabla
      return kassaEmeliyyatlar
        .filter(k => k.sifaris_id === sifaris.id && k.tip === "Mədaxil")
  ...
  const sirketler = useMemo(() => {
    return musteriler.map(m => {
      ...
      const odenilen = mSif.reduce((s, x) => s + getSifarisOdenilen(x), 0);
                ...
                <table className="w-full text-sm">
                  <thead className="bg-muted/40 sticky top-0">
                    <tr>
                      ...
                      <th className="text-right px-3 py-2 font-medium">Ümumi</th>
                      <th className="text-right px-3 py-2 font-medium text-green-700">Ödənilmiş</th>
                      <th className="text-right px-3 py-2 font-medium text-red-600">Qalıq borc</th>
                      <th className="text-left px-3 py-2 font-medium">Ödəniş</th>
                    </tr>
                  </thead>
                  <tbody>
                    {[...detay.mSif].sort((a, b) => (b.tarix || "") > (a.tarix || "") ? 1 : -1).map(s => {
                      const sifarisOdenilen = getSifarisOdenilen(s);
                      const sifarisUmumi = s.umumi_mebleg || s.qiymet || 0;
                      const sifarisQaliq = sifarisUmumi - sifarisOdenilen;
                      return (
                      <tr key={s.id} className="border-t border-border/50">
                        ...
                        <td className="px-3 py-1.5 text-right font-medium">{sifarisUmumi.toFixed(2)} ₼</td>
                        <td className="px-3 py-1.5 text-right text-green-600 font-semibold">{sifarisOdenilen.toFixed(2)} ₼</td>
                        <td className={`px-3 py-1.5 text-right font-bold ${sifarisQaliq > 0 ? 'text-red-600' : 'text-muted-foreground'}`}>{sifarisQaliq.toFixed(2)} ₼</td>
