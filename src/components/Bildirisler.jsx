...
export default function Bildirisler() {
  ...
  const fetchBildirisler = async () => {
    const data = await db.entities.Bildiris.filter({ baglandi: false }, "-tarix", 50).catch(() => []);
    
    // Check for new unread+unvoiced
    data.forEach(async b => {
      if (!b.oxundu && !b.ses_verildi && !prevIdsRef.current.has(b.id)) {
        playDing();
        await db.entities.Bildiris.update(b.id, { ses_verildi: true }).catch(() => {});
      }
    });
  ...
  const handleBagla = async (id) => {
    await db.entities.Bildiris.update(id, {
      baglandi: true,
  ...
  const handleHamisinibagla = async () => {
    await Promise.all(bildirisler.map(b =>
      db.entities.Bildiris.update(b.id, { baglandi: true, baglanis_tarixi: new Date().toISOString(), oxundu: true })
    ));
    setBildirisler([]);
    setOpen(false);
  };
