...
export default function SifarisXerciModal({ sifaris, open, onClose }) {
  ...
  const fetchXercler = () => {
    ...
    db.entities.SifarisXerci.filter({ sifaris_id: sifaris.id }).catch(() => []).then(setXercler);
  };

  useEffect(() => {
    if (open && sifaris) {
      fetchXercler();
      db.entities.Isci.filter({ status: "Aktiv" }).catch(() => []).then(setIscilar);
    }
  }, [open, sifaris]);
  ...
  const handleAdd = async () => {
    ...
    await db.entities.SifarisXerci.create({
  ...
  const handleDelete = async (id) => {
    await db.entities.SifarisXerci.delete(id);
    fetchXercler();
  };
